...
import '../styles/TopNoteSelector.css'; // Reuse same CSS

const heartNotes = {
  Floral: {
    image: process.env.PUBLIC_URL + '/images/floral.png',
    options: ['Rose', 'Jasmine', 'Lily', 'Peony'],
  },
  'Soft Floral': {
    image: process.env.PUBLIC_URL + '/images/soft-floral.png',
    options: ['Powdery', 'Aldehydes'],
  },
  'Floral Oriental': {
    image: process.env.PUBLIC_URL + '/images/floral-oriental.png',
    options: ['Orange Blossom', 'Gardenia'],
  },
  Fruity: {
  ...
  Aromatic: {
    ...
    options: ['Rosemary', 'Cardamom'],
  }
};

export default function HeartNoteSelector() {
  const [selectedHeartNote, setSelectedHeartNote] = useState(null);
  ...
  const handleSubNoteClick = (note, event) => {
    ...
    localStorage.setItem('heartNotes', JSON.stringify(updatedNotes));
  ...
  const handleRemoveNote = (note) => {
    ...
    localStorage.setItem('heartNotes', JSON.stringify(updated));
  };
  ...
  const handleNext = () => {
    navigate('/base', { state: { heartNotes: selectedSubNotes } });
  };

  return (
    <div className="top-note-container">
      <h2>Choose a Heart Note</h2>
      <div className="note-grid">
        {Object.entries(heartNotes).map(([category, data]) => (
          <div
            key={category}
            className="note-tile enlarged-tile"
            onClick={() => setSelectedHeartNote(category)}
          >
            <img src={data.image} alt={category} className="note-image" />
            <div className="note-title">{category}</div>

            {selectedHeartNote === category && (
